...
bool Screen::init() {
    ...
    if (!m_sansFont) SDL_Log("Error. Could not load font");

    m_window = SDL_CreateWindow("Snake Game", SDL_WINDOWPOS_UNDEFINED,
...
void Screen::update(int score, int lives, bool isGameOver) {
    ...
    if (!isGameOver) drawText(score, lives);
...
void Screen::drawText(int score, int lives) {
    ...
    SDL_Rect rectangle = {S_TEXT_X, S_TEXT_Y, S_TEXT_RECT_WIDTH,
                          S_TEXT_RECT_HEIGHT};
