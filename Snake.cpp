...
Snake::Snake()
    : m_speed(Snake::S_INITIAL_SPEED),
      m_lives(Snake::S_INITIAL_LIVES),
...
Snake::~Snake() { freeSections(); }

void Snake::draw(Screen& screen) {
    for (auto section : m_sections) section->draw(screen);
}
...
void Snake::freeSections() {
    for (auto section : m_sections) delete section;
}

void Snake::resetSections() {
    for (int i = S_N_SECTS; i < m_sections.size(); i++) delete m_sections[i];
...
void Snake::resetDirection() { m_direction = S_INITIAL_DIRECTION; }
...
void Snake::toString() {  // TODO Remove. For debugging purposes
    ...
    for (auto section : m_sections) section->toString();
    SDL_Log("----------------------------------------");
}
